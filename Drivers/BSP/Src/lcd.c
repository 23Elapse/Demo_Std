...
void lcd_init(void)
{
    ...
    else if (lcddev.id == 0x5310 || lcddev.id == 0x9806 || lcddev.id == 0x7796)  /* 如果是这几个IC, 则设置WR时序为最快 */
    {
        /* 重新配置写时序控制寄存器的时序 */
    ...
    else if (lcddev.id == 0x5510 || lcddev.id == 0x1963)  /* 如果是这几个IC, 则设置WR时序为最快 */
    {
        /* 重新配置写时序控制寄存器的时序 */
