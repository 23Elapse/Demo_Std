...
void W25Qxx_init(SPI_Flash_Config* config)
{
    uint8_t temp;
    if (xFlashMutex == NULL)
    xFlashMutex = xSemaphoreCreateMutex();
    if (xFlashMutex == NULL) {
        printf("Failed to create flash mutex\r\n");
        return;
    }
    ...
    {
        temp = SPI_Flash_ReadStatusReg(config, FLASH_StatusReg3);   /* 读取状态寄存器3，判断地址模式 */
        printf("status reg3 is : %d\r\n", temp);
        if ((temp & 0X01) == 0)                                 /* 如果不是4字节地址模式,则进入4字节地址模式 */
        {
            WriteEnable(config);                            /* 写使能 */
            temp |= 1 << 1;                                     /* ADP=1, 上电4位地址模式 */
            SPI_Flash_WriteStatusReg(config, FLASH_StatusReg3, temp);                         /* 写SR3 */
...
SPI_Flash_Config flash_cfg = {
    ...
    .address_mode = FLASH_4BYTE_MODE, // 默认4字节地址模式
};
...
{
    /* 参数有效性检查 */
    if(reg > FLASH_StatusReg3) return FLASH_INVALID_PARAM;
...
Flash_Status SPI_Flash_WriteStatusReg(SPI_Flash_Config* config, Flash_StatusReg reg, uint8_t sr)
{
    ...
    CS_Low(config);
    SPI_Flash_Transfer(config, g_flash_write_reg_cmd[reg]); // 选择对应命令
...
{
    if (xSemaphoreTake(xFlashMutex, portMAX_DELAY)) {
        WriteEnable(config);
        CS_Low(config);
        SPI_Flash_Transfer(config, FLASH_SectorErase);
        SendAddress(config, sectorAddr);
        CS_High(config);
        Flash_Status ret = CheckBusy(config);
        printf("[FLASH] Erase Sector 0x%08lX %s\r\n", sectorAddr, ret==FLASH_OK?"OK":"FAIL");
...
Flash_Status SPI_Flash_WritePage(SPI_Flash_Config* config, uint8_t* pBuffer, uint32_t writeAddr, uint16_t numByteToWrite)
{
    if (xSemaphoreTake(xFlashMutex, portMAX_DELAY)) {
        ...
        CS_High(config);
        Flash_Status ret = CheckBusy(config);
        printf("[FLASH] Write Page Addr:0x%08lX Len:%d %s\r\n", writeAddr, numByteToWrite, ret==FLASH_OK?"OK":"FAIL");
        xSemaphoreGive(xFlashMutex);
        return ret;
    }
    return FLASH_TIMEOUT;
